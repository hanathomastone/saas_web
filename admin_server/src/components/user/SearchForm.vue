<template>
  <form ref="searchForm" @submit.prevent="submit">
    <div class="search_row mb-5 px-8">
      ...
      <v-row no-gutters>
        <v-col class="pr-4" cols="3">
          <v-text-field
            v-model="form.userIdentifierOrName"
            background-color="inputBG"
            outlined
            height="48"
            append-icon="mdi-magnify"
            placeholder="아이디 혹은 닉네임"
            dense
    ...
    <div class="search_row mb-5 px-8">
      <p>필터</p>
      <v-row no-gutters>
        <v-col class="d-flex pr-4" cols="3">
          <select-box v-model="form.oralCheckResultTotalType" :items="oralCheckType" placeholder="최근 구강상태"></select-box>
        </v-col>
        <v-col class="d-flex pr-4" cols="3">
          <select-box v-model="form.userGender" :items="genderStatus" placeholder="성별"></select-box>
        </v-col>
        <v-col class="d-flex pr-4" cols="3">
          <select-box v-model="form.isVerify" :items="verifyStatus" placeholder="회원여부"></select-box>
        </v-col>
      </v-row>
    </div>
